...
def create_dataframe():
    ...
    pastel_colors = []
    # convertir a pastel los colores intensos
...
def add_vertical_lines(fig, ax, x_coords):
    for x in x_coords:
        ax.axvline(x=x, color='red', linestyle='--')
    return fig

def create_line_plot(df, theme):
    global fig, ax1
    ...
    # Line plot
    umbral_y = 0.2
    plt.plot(df, color=theme.text_color)
...
if __name__ == "__main__":
    import random
    # from tools time series import coso
    # Usage
    # df = create_dataframe()
    ...
    random_choices = random.sample(df.index.tolist(), 11)
    fig = add_vertical_lines(fig, ax1, random_choices)
